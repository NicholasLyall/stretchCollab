...
class OpenLoopGraspObjectOperation(ManagedOperation):
    ...
    def run(self):
        ...
        print("🟡🟡🟡 OpenLoopGraspObjectOperation.run() STARTED! 🟡🟡🟡")
        self.attempt("Grasping an object")

        # Speech announcement for positioning phase
        if hasattr(self.agent, 'robot_say') and hasattr(self.agent, 'enable_speech_debug') and self.agent.enable_speech_debug:
            self.agent.robot_say(f"Positioning to grasp {self.target_object}")
        ...
        if hasattr(self.agent, 'robot_say') and hasattr(self.agent, 'enable_speech_debug') and self.agent.enable_speech_debug:
            self.agent.robot_say(f"Grasping {self.target_object}")
    ...
    def grasp_open_loop(self, object_xyz: np.ndarray):
        ...
        # Speech announcement for grasp completion
        if hasattr(self.agent, 'robot_say') and hasattr(self.agent, 'enable_speech_debug') and self.agent.enable_speech_debug:
            self.agent.robot_say(f"Grasp complete. Now holding {self.target_object}")
