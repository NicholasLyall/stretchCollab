...
class PreGraspObjectOperation(ManagedOperation):
    ...
    def can_start(self):
        ...
        print(f"🎯 PREGRASP DISTANCE CHECK: Object at {dist:.3f}m, threshold: {self.grasp_distance_threshold}m")
    ...
    def run(self):
        ...
        if hasattr(self.agent, 'robot_say'):
            try:
                if hasattr(self.agent, 'target_object') and self.agent.target_object:
                    self.agent.robot_say(f"Positioning for grasp of {self.agent.target_object}")
                else:
                    self.agent.robot_say("Positioning for precise grasp")
            except:
                pass
        ...
        # Simple higher start: go 0.2m higher first, then come down to 0.1m higher than normal
        print("🔼 Going 0.2m higher, then down to 0.1m higher than normal pregrasp position")
        higher_joint_state = joint_state.copy()
        higher_joint_state[HelloStretchIdx.LIFT] += 0.2  # 20cm higher
        
        # Make final position 0.1m higher than originally calculated
        joint_state[HelloStretchIdx.LIFT] += 0.1  # Final position is 10cm higher than normal
        ...
        # Step 1: Go 0.2m higher first
        self.robot.arm_to(higher_joint_state, blocking=True)
        
        # Step 2: Then come down to 0.1m higher than original target
