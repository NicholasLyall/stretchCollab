...
# Import enhanced grasp operation for improved accuracy and reliability
try:
    from stretch.agent.operations.enhanced_grasp_object import EnhancedGraspObjectOperation
    ENHANCED_GRASP_AVAILABLE = True
except ImportError:
    ENHANCED_GRASP_AVAILABLE = False
    print("[GRASP] Enhanced grasp operation not available, using standard approach")

# Also try advanced visual servoing as secondary option
try:
    from stretch.agent.operations.visual_servo_grasp import VisualServoGraspOperation
...
class GraspObjectOperation(ManagedOperation):
    """Move the robot to grasp, using the end effector camera with visual servoing."""

    use_pitch_from_vertical: bool = True
    lift_distance: float = 0.2
    servo_to_grasp: bool = True  # Default to visual servoing for better accuracy
    _success: bool = False
    talk: bool = True
    verbose: bool = False
    
    # Import visual servoing for fallback and integration
    use_visual_servoing: bool = True
    ...
    def _grasp(self, distance: Optional[float] = None) -> bool:
        ...
        if self.talk:
            self.agent.robot_say("Closing gripper")
    ...
    def visual_servo_to_object(
        ...
        if self.talk:
            self.agent.robot_say("Beginning visual servoing")
        print("🔥🔥🔥 CALLING pregrasp_open_loop() - MY HIGH RESET CODE SHOULD RUN NOW! 🔥🔥🔥")
    ...
    def run(self) -> None:
        print("🔥🔥🔥 GraspObjectOperation.run() STARTED! 🔥🔥🔥")
        ...
        # NOTE: Visual servoing is now handled in the enhanced navigation/pregrasp phases
        # The final grasp operation uses standard approach to avoid jerky transitions
        
        print("🔥 GRASP: Using standard final grasp (visual servoing handled in earlier phases).")
        self.info("Using standard final grasp (visual servoing handled in earlier phases).")
        ...
        if self.servo_to_grasp:
            ...
            print("🔥🔥🔥 CALLING visual_servo_to_object() - THIS IS WHERE HIGH RESET SHOULD HAPPEN! 🔥🔥🔥")
    ...
    def pregrasp_open_loop(self, object_xyz: np.ndarray, distance_from_object: float = 0.35):
        ...
        """
        print("🔥🔥🔥 pregrasp_open_loop() STARTED! 🔥🔥🔥")
        xyt = self.robot.get_base_pose()
        relative_object_xyz = point_global_to_base(object_xyz, xyt)
        ...
        # Now move to the calculated pregrasp position (which includes extension)
