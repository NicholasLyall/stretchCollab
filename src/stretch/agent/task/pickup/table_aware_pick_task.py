...
class TableAwarePickObjectTask:
    """
    Height-adaptive robot task that will search for an object at any height and pick it up safely.
    ...
    def __init__(
        ...
        """Initialize height-adaptive pick task.
        ...
        logger.info(f"[TABLE-AWARE PICK] Initialized height-adaptive pick task for '{target_object}'")
        logger.info(f"[TABLE-AWARE PICK] Safety margin: {safety_margin}m, Visual servoing: {use_visual_servoing_for_grasp}")
    ...
    def get_task(self, add_rotate: bool = False) -> Task:
        ...
        Returns:
            Task: Executable task plan for the robot to pick up objects at any height safely.
        """
        return self.get_height_adaptive_task(add_rotate=add_rotate, matching=self.matching)

    def get_height_adaptive_task(self, add_rotate: bool = False, matching: str = "feature") -> Task:
        """Create a height-adaptive task plan that works for objects at any elevation."""
        ...
        # Use regular search (no conflicting table-aware wrapper)
        from stretch.agent.operations import SearchForObjectOperation
        search_for_object = SearchForObjectOperation(
        ...
        # Navigate to detected object (handles any height)
        ...
        # Height-adaptive pregrasp positioning
...
if __name__ == "__main__":
    ...
    task = TableAwarePickObjectTask(agent, target_object="apple").get_task(add_rotate=False)
    task.run()