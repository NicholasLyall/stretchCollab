...
# Import enhanced pickup task
try:
    from stretch.agent.task.pickup.enhanced_pickup_task import EnhancedPickupTask
    ENHANCED_PICKUP_AVAILABLE = True
except ImportError:
    ENHANCED_PICKUP_AVAILABLE = False
    print("[PICKUP EXECUTOR] Enhanced pickup task not available, using standard approach")
...
class PickupExecutor:
    ...
    def __init__(
        ...
        # Log configuration
        if self._enhanced_visual_servoing and not self._open_loop and ENHANCED_PICKUP_AVAILABLE:
            logger.info("[PICKUP] Enhanced visual servoing enabled in navigation/approach phases!")
        elif not self._open_loop:
            logger.info("[PICKUP] Standard visual servoing enabled")
        else:
            logger.info("[PICKUP] Open-loop grasping enabled (visual servoing disabled)")
    ...
    def _pickup(self, target_object: str, target_receptacle: str) -> None:
        ...
        # LOUD DEBUG SPEECH  
        self.agent.robot_say("USING BASIC PICKUP AND PLACE!")
        
        logger.alert(f"[Pickup task] Pickup: {target_object} Place: {target_receptacle}")

        # After the robot has started...
        try:
            # DISABLED: Enhanced pickup task is causing grasp failures
            # Always use standard pickup task for reliability
            logger.info("[PICKUP] Using standard pickup task (enhanced operations disabled)")
    ...
    def _pick_only(self, target_object: str) -> None:
        ...
        # LOUD DEBUG SPEECH
        self.agent.robot_say("USING BASIC PICKUP!")
        
        logger.alert(f"[Pickup task] Pickup: {target_object}")

        # After the robot has started...
        try:
            # Enhanced pickup is for full pickup+place, so use standard for pick-only
            # but we can still use enhanced visual servoing if a pick-only enhanced task is created
    ...
    def _place(self, target_receptacle: str) -> None:
        ...
        """
        # LOUD DEBUG SPEECH
        self.agent.robot_say("USING BASIC PLACE!")
