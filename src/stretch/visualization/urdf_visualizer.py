...
class URDFVisualizer:
    ...
    def get_combined_robot_mesh(self, cfg: dict = None, use_collision: bool = True) -> Trimesh:
        ...
        # Handle different trimesh versions - remove_duplicate_faces() may not exist
        try:
            combined_mesh.remove_duplicate_faces()
        except AttributeError:
            # Method doesn't exist in this trimesh version, try alternatives
            try:
                combined_mesh.remove_degenerate_faces()
            except AttributeError:
                # If neither method exists, continue without face cleaning
                pass
